...
public class HapusKaryawanInternal extends JInternalFrame {



    public HapusKaryawanInternal() {
        initComponents();
        pack();
    ...
    private void initComponents() {
        ...
        btnHapusSemua.setText("Hapus SEMUA Data Karyawan");
    ...
    private void hapusSatu() {
        ...
        if (id.equals(Session.getIdKasir())) {
            JOptionPane.showMessageDialog(this, "Tidak dapat menghapus akun yang sedang digunakan untuk login.");
            return;
        }

        int confirm = JOptionPane.showConfirmDialog(this, "Hapus karyawan \"" + selected + "\"?",
                "Konfirmasi Hapus", JOptionPane.YES_NO_OPTION);
        if (confirm != JOptionPane.YES_OPTION) return;

        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM kasir WHERE id_kasir=?")) {
            ps.setString(1, id);
            ps.executeUpdate();
    ...
    private void hapusSemua() {
        int confirm = JOptionPane.showConfirmDialog(this,
                "Hapus SEMUA data karyawan selain akun yang sedang login (" + Session.getNamaKasir() + ")?",
                "Konfirmasi Hapus Semua", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.YES_OPTION) return;

        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM kasir WHERE id_kasir <> ?")) {
            ps.setString(1, Session.getIdKasir());
            ps.executeUpdate();
            JOptionPane.showMessageDialog(this, "Seluruh data karyawan (selain akun Anda) berhasil dihapus.");
            loadIds();
        } catch (SQLException ex) {
